...
| 命令 | 功能 | 示例 |
|------|------|------|
| `.deepseek status` | 查看系统状态 | `.deepseek status` |
| `.deepseek config` | 查看详细配置 | `.deepseek config` |
| `.deepseek help` | 查看管理帮助 | `.deepseek help` |

#### 用户管理
| 命令 | 功能 | 示例 |
|------|------|------|
| `.deepseek users` | 查看用户列表 | `.deepseek users` |
| `.deepseek user <用户ID>` | 查看用户详情 | `.deepseek user 123456` |
| `.deepseek user lock <用户ID>` | 锁定用户 | `.deepseek user lock 123456` |
| `.deepseek user unlock <用户ID>` | 解锁用户 | `.deepseek user unlock 123456` |
| `.deepseek user clear <用户ID>` | 清空用户记录 | `.deepseek user clear 123456` |

#### 系统配置
| 命令 | 功能 | 示例 |
|------|------|------|
| `.deepseek set cooldown <秒数>` | 设置冷却时间 | `.deepseek set cooldown 15` |
| `.deepseek set context <段数>` | 设置上下文限制 | `.deepseek set context 10` |
| `.deepseek set prefix <前缀>` | 设置触发前缀 | `.deepseek set prefix /ai` |
| `.deepseek set tokens <数量>` | 设置最大token数 | `.deepseek set tokens 2000` |
| `.deepseek set temperature <数值>` | 设置温度参数 | `.deepseek set temperature 0.8` |
| `.deepseek set model <模型名>` | 设置AI模型 | `.deepseek set model deepseek-coder` |
| `.deepseek set apikey <key>` | 设置API Key | `.deepseek set apikey sk-xxx` |
| `.deepseek set endpoint <url>` | 设置API端点 | `.deepseek set endpoint https://...` |

#### 功能开关
| 命令 | 功能 | 示例 |
|------|------|------|
| `.deepseek toggle group` | 切换群聊功能 | `.deepseek toggle group` |
| `.deepseek toggle private` | 切换私聊功能 | `.deepseek toggle private` |
| `.deepseek toggle debug` | 切换Debug模式 | `.deepseek toggle debug` |

#### 系统维护
| 命令 | 功能 | 示例 |
|------|------|------|
| `.deepseek reset` | 重置系统配置 | `.deepseek reset` |
| `.deepseek cleanup` | 清理过期数据 | `.deepseek cleanup` |
